...
pub fn start(
    ...
    let (_position_tx, position_rx) = mpsc::channel::<PositionOpened>(CHANNEL_CAPACITY);
    let (_alert_tx, alert_rx) = mpsc::channel::<PostBuyAlert>(CHANNEL_CAPACITY);

    // Keep the public API in place for the redesign, but intentionally disable
    // every buy path. Main no longer starts this engine; if any future caller
    // wires it accidentally, it only drains and logs filtered tokens.
    let _ = (cfg, supabase, wallet, trading_state);

    tokio::spawn(async move {
        info!("Execution skeleton started — buy logic disabled, draining filter channel only");

        while let Some(token) = filter_rx.recv().await {
            let mint_str = token.event.mint.to_string();
            info!(mint = %mint_str, "🧩 Execution skeleton skipped filtered token — no buy attempted");
        }

        info!("Execution skeleton shutting down (filter channel closed)");
    });

    (position_rx, alert_rx)
}
